...
get_cloudfront_dist_alias() {
  ...
  CLOUDFRONT_DIST_ALIAS=$(aws cloudfront get-distribution --id "$CLOUDFRONT_DIST_ID" --profile commit-bucket-deploy | jq -r '.Distribution.DistributionConfig.Aliases.Items[0]')
